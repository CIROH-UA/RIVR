...
describe("the digest windows to what is ahead, as the app does", () => {
  test("past points are dropped", () => {
    const out = upcomingFrom([p(-6, 500), p(-1, 400), p(2, 100)], NOW);
    assert.deepEqual(out.map((x) => x.value), [100]);
  });

  test("a point exactly at now counts as ahead", () => {
  ...
  test("an entirely lapsed series falls back to the whole thing", () => {
    const all = [p(-8, 3), p(-4, 2)];
    assert.deepEqual(upcomingFrom(all, NOW), all);
  });
  ...
  test("a river that already crested reads the same on both sides", () => {
    ...
    assert.equal(peak > ahead[0].value * 1.05, false,
      "windowed forward it is not rising — this is the White River case");
  ...
  test("the peak reported is one that has not happened yet", () => {
    ...
    assert.equal(peak.value, 300,
      "the 5000 crest is behind us; announcing it would be wrong");
    assert.ok(Date.parse(peak.validTime) >= NOW.getTime());
  });
});
