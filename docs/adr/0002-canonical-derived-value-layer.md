...
**Stage 2 parked, gated on the next source.** Canonical current‑flow (audit #4/#5), single return‑period source (audit #6/#7), and the source‑agnostic `RiverForecast` model that retires the `ForecastResponse` fork (ADR 0001 Step 7) are the remaining work. They are sequenced when a new data source actually carries forecast *detail* — that's the forcing function that makes the generalization concrete rather than speculative, and it's the point at which not having it would force a fork.
...
2. **Canonical current‑flow** — one function that picks the "now" value from repository data; retire the per‑surface/per‑cache "now" pickers (audit #4/#5).
3. **Canonical flow formatting** — one `formatFlow(value, unit)`; remove ad‑hoc number/label formatting in widgets.
4. **Source‑agnostic `RiverForecast`** — retire the `ForecastResponse`/`GeoglowsForecast` fork so detail pages derive from a uniform, capability‑based model (= ADR 0001 Step 7). Largest/riskiest; last; gated on the next detail‑carrying source.
